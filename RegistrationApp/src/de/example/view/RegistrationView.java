...
public class RegistrationView extends JFrame 
{
	...
	//Labels
	 ...
	 { 
		welcomeMessage = new JLabel  ("Mister Diallo App. Please enter your informations");
		welcomeMessage.setBounds(100,0,1600,100);
		welcomeMessage.setFont(new Font("Serif", Font.PLAIN, 60));
	...
	private void save() 
	{
		//Check the fields if they are empty
	...
	private void openUserList() 
	{
		...
		jScrollPane.setBounds(0,0,1550,600); 
	...
	private String[][] createDataVector(List<UserModell> userList)
		{
			...
			String[][] rows= new String[userList.size()][9];
			int index = 0;
			for(UserModell user : userList)
			{
				theID++; //The value will increase after every adding process
			
				String[] row = new String[9]; //the number of informations to display
				
					//DIese Schreibweise erlaubt die einzelne Elemente in die Tabelle zu zeigen
					row[0] = Integer.toString(theID);
					row[1] = user.getName();
					row[2] = user.getFirstName();
					row[3] = user.getGender();
					row[4] = user.getAge();
					row[5] = user.getCountry();
					row[6] = user.getCity();
					row[7] = user.getEmail();
